...
export const AdvisorContextProvider = ({children}) =>{
    ...
    const postAdvisorDetails = async() =>{
        try {
            const response = await fetch("http://localhost:3000/api/agro-advisor?lon=8.1&lat=1.2", {
                method: "Post",
                headers: {
    ...
    return(
        <advisorContext.Provider value={{fieldValuesChange, feildsValues, postAdvisorDetails}}>
            {children}
